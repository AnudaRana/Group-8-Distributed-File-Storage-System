...
func (r *Raft) electionTimeout() time.Duration {
	// 4-8 seconds: wide range prevents split-vote loops at startup
	// Heartbeat is 500ms â†’ at least 8x margin
	return time.Duration(4000+rand.Intn(4000)) * time.Millisecond
}
...
func (r *Raft) startElection() {
	...
	// Safely copy peers to avoid crash if AddPeer mutates them concurrently
	peersCopy := make([]string, len(r.PeerURLs))
	copy(peersCopy, r.PeerURLs)
