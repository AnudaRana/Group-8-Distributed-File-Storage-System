...
// NewSyncer instantiates an orchestrator for continuously polling,
// measuring, and enforcing clock skew adjustments against a master peer.
func NewSyncer(peerURL string) *Syncer {
	return &Syncer{peerURL: peerURL}
}

// collectOneSample executes a single timing volley to a peer, returning
// the outbound timestamp, remote server timestamp, and local return timestamp.
...
// CollectSamples executes multiple timing volleys in sequence, discarding
// any samples that exceed acceptable Round Trip Time (RTT) thresholds.
...
// Synchronise performs a full synchronization burst against the remote peer,
// selecting the median skew and adjusting the local software clock offset.
...
// Now acts as a drop-in replacement for time.Now(), returning the local time
// adjusted by the computed clock skew to cleanly align with the cluster master.
...
// Offset returns the raw nanosecond delta currently being applied to the local clock.
...
// OffsetSeconds returns the identical offset delta represented in fractional seconds.
func (s *Syncer) OffsetSeconds() float64 {
	return float64(s.Offset()) / 1e9
}

// SkewHistory yields the telemetry records detailing how the system's clock drift
// has shifted and accumulated over previous synchronization evaluation epochs.
...
// HandleSyncFailure provides fallback logic when the target peer is unreachable,
// attempting to maintain consistency by degrading to the last successfully measured offset.
...
// RunLoop spins up a robust background goroutine that perpetually synchronizes 
// the clock at the requested interval until an explicit stop channel trigger is received.
...
// GetSyncedTime behaves as a convenience wrapper to fetch an accurate point-in-time
// directly from a remote peer orchestrating a one-shot volley and returning its calculation.
