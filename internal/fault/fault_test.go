...
// Test 2: End-to-end â€” failure detected and callback fires
func TestFaultManagerDetectsFailure(t *testing.T) {
	...
	fm.Detector.OnFailure = func(nodeID, addr string) {
		mu.Lock()
		failedNode = nodeID
		mu.Unlock()
	}
...
// Test 5: FaultManager â€” node stays alive if heartbeats are sent
...
// Test 6: FaultManager â€” multiple nodes, only silent one fails
...
// Test 7: FaultManager â€” callback fires exactly once per failure
...
// Test 8: FaultManager â€” node recovery after rejoining
func TestFaultManagerNodeRecovery(t *testing.T) {
	...
	// Simulate node2 coming back â€” send a heartbeat
	fm.Detector.RecordHeartbeat("node2")
